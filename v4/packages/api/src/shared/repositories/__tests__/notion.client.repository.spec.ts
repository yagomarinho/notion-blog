...
describe('NotionClientRepository', () => {
  ...
  it('should be able to call getPage, extract properties and convert the result', async () => {
    ...
    const id = 'abcd-1234'
    const rawPage: ExtendedRecordMap = { recordMap: { blocks: { [id]: {} } } }

    getPageMock.mockResolvedValueOnce(rawPage)
    ...
    expect(extractPropertiesFromRecordMap).toHaveBeenCalledWith(rawPage, id)
